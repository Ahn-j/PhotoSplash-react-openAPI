...
function DetailDialog({ data, handleChange }: Props) {
  ...
  const handleClick = () => {
    ...
    if (!getLocalStorage || getLocalStorage === null) {
      ...
      if (
        ...
        alert("북마크에서 삭제합니다");
        setBookMark(false);
        const remove = getLocalStorage.filter(
          (item: CardDTO) => item.id !== data.id
        );
      ...
      <div className={styles.container__dialog}>
        <div className={styles.container__dialog__header}>
          ...
          <div className={styles.bookmark}>
            <button className={styles.bookmark___button} onClick={handleClick}>
              {bookmark === false ? (
                <span
                  className="material-symbols-outlined"
                  style={{ fontSize: "16px" }}
            ...
            <button className={styles.bookmark___button}>downLoad</button>
          </div>
        </div>
